...
echo "<h2>🔄 สร้างตาราง Orders ใหม่ (แก้ไขปัญหา)</h2>";
...
try {
    ...
    // ลบตาราง payments ที่มีอยู่ (ถ้ามี)
    echo "🗑️ กำลังลบตาราง payments เดิม...<br>";
    $dropPaymentsQuery = "DROP TABLE IF EXISTS payments CASCADE";
    $dropPaymentsResult = pg_query($conn, $dropPaymentsQuery);
    if ($dropPaymentsResult) {
        echo "✅ ลบตาราง payments สำเร็จ<br>";
    }
    ...
    // ตรวจสอบตารางทั้งหมด
    echo "<br><h3>📊 ตารางทั้งหมดในฐานข้อมูล:</h3>";
    $checkAllTablesQuery = "
        SELECT table_name 
    ...
    $checkAllTablesResult = pg_query($conn, $checkAllTablesQuery);
    if ($checkAllTablesResult) {
        echo "<ul>";
        while ($row = pg_fetch_assoc($checkAllTablesResult)) {
