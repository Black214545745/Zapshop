<?php
echo "<h2>รีเซ็ตฐานข้อมูล PostgreSQL</h2>";
...
foreach ($drop_tables as $sql) {
    ...
    if ($result) {
        echo "<p style='color: blue;'>✓ ลบตารางเก่าสำเร็จ</p>";
    } else {
        echo "<p style='color: orange;'>⚠ ไม่มีตารางเก่าให้ลบ</p>";
    }
}

echo "<h3>สร้างตารางใหม่:</h3>";
...
if ($result) {
    ...
    echo "<p style='color: red;'>✗ เพิ่มหมวดหมู่ล้มเหลว: " . pg_last_error($conn) . "</p>";
}
...
if ($result) {
    ...
    echo "<p style='color: red;'>✗ เพิ่มสินค้าล้มเหลว: " . pg_last_error($conn) . "</p>";
}
...
if ($result) {
    ...
    echo "<p style='color: red;'>✗ เพิ่มผู้ดูแลระบบล้มเหลว: " . pg_last_error($conn) . "</p>";
}
...
if ($result) {
    echo "<p><strong>ตาราง users:</strong></p>";
    echo "<ul>";
...
echo "<p style='color: green; font-weight: bold;'>ฐานข้อมูลถูกรีเซ็ตและสร้างใหม่เรียบร้อยแล้ว</p>";
